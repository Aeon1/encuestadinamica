...
<input type="hidden" id="idstorage" value="encuesta{{ $encuesta['encuesta']['id'] }}">
  <div class="container cpe" style="padding-top:120px; background: whitesmoke;">
    ...
    <fieldset id="tab0" class="collapse">
      ...
      <form class="needs-validation" id="{{App\Helpers::generate_string(15)}}" novalidate data-envio="{{ !empty($encuesta['secciones'])?'Continuar':'Finalizar'}}">
            ...
            @foreach ($encuesta['preguntas'] as $k=>$pregunta)
              @if ($pregunta['seccion']== $seccion['seccion'])
                ...
                @if ($pregunta['tipo'] == 4)
                  @if ($pregunta['asignacion'] == 0)
                    <div class="form-group principal" data-nivel="{{$pregunta['subnivel']}}">
                      <label for="x{{$pregunta['name']}}">{{$pregunta['pregunta']}}</label>
