@extends('appPublicacionIns')
@section('main')
<nav class="navbarx" style="top: 60px !important;">
  <div class='container'>
      <div class='barras'>
...
<input type="hidden" id="idstorage" value="encuesta{{ $encuesta['encuesta']['id'] }}">
  <div class="container cpe" style="padding-top:120px; background: whitesmoke;">
    @if (!empty($encuesta['instrucciones']))
      <a href="#" class="btn-flotante" data-toggle="modal" data-target="#instruccionesModal" title="Ver Instrucciones"><i class='glyphicon glyphicon-info-sign'></i></a>
    @endif
    <a class="btn-flotante-save" id="savex" title="Guardar avance, para retomar después"><i class="glyphicon glyphicon-floppy-disk" aria-hidden="true"></i></a>
    {{-- registro --}}
    <fieldset id="tab0" class="collapse">
      <legend>Datos de registro</legend>
      <form role="form" data-toggle="validator" id="{{App\Helpers::generate_string(15)}}" data-envio="{{ !empty($encuesta['secciones'])?'Continuar':'Finalizar'}}">
    ...
    @if (!empty($encuesta['secciones']))
      @foreach ($encuesta['secciones'] as $seccion)
        <fieldset id="tab{{$seccion['seccion']}}" class="collapse">
          ...
          <form role="form" data-toggle="validator" id="{{App\Helpers::generate_string(20)}}" data-envio="{{ $seccion['seccion']==count($encuesta['secciones'])?'Finalizar':'Continuar' }}">
